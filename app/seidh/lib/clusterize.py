...
def clusterize(sorted_ids,folder):
    N = len(sorted_ids)
...
def write_cluster_info(clusters, energies):
    with open('cluster.repr', 'w') as output:
        for id_cluster, ids in clusters.iteritems():
            output.write("%d:%d:%8.5f:%d:%s\n" % (id_cluster, len(ids), energies[ids[0]],
                                                  ids[0], '%d.pdb' % ids[0]))
        

def find_clusters(pdb_ids,folder):

    clusters = clusterize(pdb_ids,folder)
    return clusters
    #write_cluster_info(clusters, energies)