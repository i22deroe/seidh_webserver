...
from constants.constants import PROTEINS,PEPTIDES,PREDICTIONS,LOG,ALLOWED_EXTENSIONS
import random
...
def structure_adaptation(structure,model,chain,entity,name):
	if entity=="protein":
		structure[model][chain].id='A'
		structure[model].id=0
		io=PDBIO()
		io.set_structure(structure[model][chain])
		io.save(PROTEINS+name)
		return True
	if entity=="peptide":
		structure[model][chain].id='A'
		structure[model].id=0
...
def open_fasta_peptide(file):
	...
	'''
	if not os.path.isfile(PEPTIDES+file+'.fas'):
		return fasta_ambiguity(file)
	else:
		fasta_file = open(PEPTIDES+file+'.fas','r')
...
def open_pdb_entity(file,chain):
	...
	if not os.path.isfile(ENTITIES+"pdb"+file+".ent"):
		...
		parser=PDBParser()
		structure=parser.get_structure(file,ENTITIES+"pdb"+file+".ent")
...
def del_file(entity,name):
	if entity=="protein":
		if os.path.exists(PROTEINS+name):
			os.remove(PROTEINS+name)
			return True
		else:
			return False
	if entity=="peptide":
		if os.path.exists(PEPTIDES+name):
			os.remove(PEPTIDES+name)
			return True
		else:
			return False
	if entity=="prediction":
		if os.path.exists(PREDICTIONS+name):
			os.remove(PREDICTIONS+name)
			return True
		else:
			return False
	return False
